...
export function FaceEmotionDetector({
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        setError(`Failed to load AI models: ${err instanceof Error ? err.message : "Unknown error"}`)
        setLoadingProgress("")
  ...
  const generateMockEmotions = (): EmotionData => {
    ...
    const primaryEmotion = emotions[Math.floor(Math.random() * emotions.length)] as keyof EmotionData
    mockData[primaryEmotion] = 0.6 + Math.random() * 0.3 // 60-90%

    // Distribute remaining probability
    const remaining = 1 - mockData[primaryEmotion]
    const otherEmotions = emotions.filter((e) => e !== primaryEmotion) as (keyof EmotionData)[]

    otherEmotions.forEach((emotion, index) => {
      if (index === otherEmotions.length - 1) {
        // Last emotion gets remaining probability
        mockData[emotion] = Math.max(0, remaining - otherEmotions.slice(0, -1).reduce((sum, e) => sum + mockData[e], 0))
      } else {
      ...
      if (detections.length > 0) {
        ...
        if (ctx) {
          ...
          resizedDetections.forEach((detection: any) => {
            ...
            ctx.font = "14px Arial"
            ctx.fillText(`${topExpression[0]}: ${(topExpression[1] * 100).toFixed(0)}%`, x, y - 10)
          })
        }
      } else {
  ...
  return (
    <div className="relative">
      ...
      {/* Status indicators */}
      {error && (
        <div className="absolute top-2 left-2 bg-red-500 text-white px-2 py-1 rounded text-xs max-w-xs">{error}</div>
      )}

      {loadingProgress && (
        <div className="absolute top-2 left-2 bg-blue-500 text-white px-2 py-1 rounded text-xs">{loadingProgress}</div>
      )}
